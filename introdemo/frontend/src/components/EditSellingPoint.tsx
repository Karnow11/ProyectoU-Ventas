...
const EditSellingPoint = ({ sellingPoint }: Props) => {
  const [name, setName] = useState<string>(sellingPoint.name)
  const [static_dynamic, setStatic_dynamyc] = useState<boolean>(sellingPoint.static_point)
  const [product_type, setProduct_type] = useState<ProductType>(sellingPoint.product_type)
  const [description, setDescription] = useState<string>(sellingPoint.description)
  const [zone, setZone] = useState<SPZone | "">(sellingPoint.zone || "")
  const navigate = useNavigate();

  const { fetchSP } = SPStore();

  const updateSP = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    await api.put(`/api/selling_points/${sellingPoint.id}`, {
      name, 
    ...
    await fetchSP();
    navigate(`/sellingPoint/${sellingPoint.id}`);
  }
  ...
  return (
    ...
    <div className='formSP-container'>
      ...
      <form onSubmit={updateSP} className = "formSP">
        ...
        <button type="button" onClick={() => navigate(`/sellingPoint/${sellingPoint.id}`)}>Cancelar</button>
      </form>
    </div>
    </>
  )
...
export default EditSellingPoint;