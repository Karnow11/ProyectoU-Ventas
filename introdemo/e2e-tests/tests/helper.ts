...
const registerWith = async (page: Page, username: string, email: string, password: string) => {
...
const createPost = async (page: Page, store_name: string, movility = "static", product_type: string, descrip: string) => {
    ...
    if movility = "static":
        await page.getByRole('radio', { name: 'Estático' }).click()
    else: 
        await page.getByRole('radio', { name: 'Móvil' }).click()
